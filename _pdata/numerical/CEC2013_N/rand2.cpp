...
#include <windows.h>
#include <wincrypt.h>
 // 模拟 nrand48 函数（windows下缺失）
unsigned long nrand48(unsigned short xsubi[3]) {
    // 定义常量
    constexpr uint64_t a = 0x5deece66dull; // 乘数
    constexpr uint64_t c = 0xbull;         // 增量
    constexpr uint64_t mask = 0xffffffffffffull; // 48位掩码

    // 将 xsubi 的三个16位值组合成一个48位的值
    uint64_t state = (static_cast<uint64_t>(xsubi[2]) << 32) |
    ...
    // 将新的状态值拆分为三个16位值，存回 xsubi
    xsubi[0] = state & 0xffff;
    xsubi[1] = (state >> 16) & 0xffff;
    xsubi[2] = (state >> 32) & 0xffff;

    // 返回结果（31位）
    return static_cast<unsigned long>(state >> 17);
}
#endif
...
static void initialize (void)
{
  ...
  {
#if defined(_WIN32) || defined(_WIN64)
      HCRYPTPROV hCryptProv;  // 加密服务提供程序句柄
      long rn[N_tables * Table_size];  // 存储随机数的缓冲区

      // 初始化加密服务提供程序
      if (!CryptAcquireContext(&hCryptProv, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT)) {
          fprintf(stderr, "Error initializing cryptographic provider.\n");
          exit(1);
      }

      // 使用 CryptGenRandom 生成随机数据
      if (!CryptGenRandom(hCryptProv, sizeof(rn), (BYTE*)rn)) {
          fprintf(stderr, "Error generating random numbers.\n");
          CryptReleaseContext(hCryptProv, 0);  // 释放资源
          exit(1);
      }
