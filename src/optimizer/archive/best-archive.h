//------------------------Description------------------------
// 最优档案 BestArchive:记录优化器/种群运行过程中的最优(或可接受)解。
//-------------------------Copyright-------------------------
...
namespace ECFlow
{
    ...
    public:
        ...
        virtual Solution* getBest() { return _best_solution; }

        virtual Solution* getElite() = 0;

        virtual bool updateBest(const Solution& new_best) = 0;

        virtual int getBestSize() { return _best_size; }

        virtual void clear() { _best_size = 0; }
    ...
    {
    public:
        BasicArchive() { _best_solution = new Solution(); }

        ~BasicArchive() { delete _best_solution; }

        Solution* getElite() { return _best_solution; }
    ...
    // 偏序最优解档案管理器（自定义尺寸）
    // 注:未注册——setProblem 为空、_comparer_pointer 保持 nullptr(原工厂即未纳入)。见 T-ARCH。
    ...
    public:
        ...
        ~HistoricArchive() { delete[] _best_solution; }

        Solution* getElite() { return _best_solution + ECFlow::get_int(0, _best_size - 1); }
    ...
    public:
        ...
        ~MultiobjectArchive() { delete[] _best_solution; }
    ...
    private:
        ...
        bool isEffect(const Solution& a, const Solution& b)   // b by ref: Solution has no deep copy ctor
        {                                                     //   (Rule-of-Three); by-value double-freed new_best
            // 存在明显差异或是相同峰
    ...
    public:
        ...
        ~MultimodalArchive() { delete[] _best_solution; }
    ...
    // 自注册进 Registry<BestArchive>(T_bestarchive;取代原 ArchiveFactory 的 produce 分派)。
    // 忠实原工厂:仅 Basic/Multiobject/Multimodal 三档;均无参(默认构造);无前/后置断言。
    inline Registry<BestArchive>::Entry basicArchiveEntry()
    {
        return { "Basic", ModuleType::T_bestarchive, ParameterTemplate{}, sizeof(BasicArchive),
            [](const double*) -> BestArchive* { return new BasicArchive(); },
            [](AssertList&, const double*) {}, [](AssertList&, const double*) {} };
    }
    inline Registry<BestArchive>::Entry multiobjectArchiveEntry()
    {
        return { "Multiobject", ModuleType::T_bestarchive, ParameterTemplate{}, sizeof(MultiobjectArchive),
            [](const double*) -> BestArchive* { return new MultiobjectArchive(); },
            [](AssertList&, const double*) {}, [](AssertList&, const double*) {} };
    }
    inline Registry<BestArchive>::Entry multimodalArchiveEntry()
    {
        return { "Multimodal", ModuleType::T_bestarchive, ParameterTemplate{}, sizeof(MultimodalArchive),
            [](const double*) -> BestArchive* { return new MultimodalArchive(); },
            [](AssertList&, const double*) {}, [](AssertList&, const double*) {} };
    }
    ECFLOW_REGISTER(arch_basic,       BestArchive, basicArchiveEntry());
    ECFLOW_REGISTER(arch_multiobject, BestArchive, multiobjectArchiveEntry());
    ECFLOW_REGISTER(arch_multimodal,  BestArchive, multimodalArchiveEntry());
}
